...
export default function Navbar({
  ...
  activeItemClassName = "",
  hoverItemClassName = "",
  bracketColor: customBracketColor,
  ...
  const borderColorClass = customBorderColor
    ...
    ? "border-white/50"
    : "border-black/50";
  const borderColorStyle = customBorderColor
    ? { borderColor: customBorderColor }
    : undefined;
  const textColorClass = customTextColor
    ...
    ? "text-white"
    : "text-black";
  const textMutedColorClass = customTextColor
    ? ""
    : isDark
    ? "text-white/70"
    : "text-black/70";
  // Highlight color for home button (icon, not text)
  const highlightBgClass = highlightColor
    ? ""
    : isDark
    ? "bg-white/20"
    : "bg-black/70";
  const highlightBgStyle = highlightColor
    ? { backgroundColor: highlightColor }
    : undefined;
  const homeHoverBgClass = highlightColor
    ? ""
    : isDark
    ? "hover:bg-white/20"
    : "hover:bg-black/70";
  const homeHoverBgStyle = highlightColor ? undefined : undefined; // Will be handled by the span inside
  const homeIconColor = customTextColor
    ? ""
    : isDark
    ? "text-white"
    : "text-black";
  ...
  // Custom color styles
  const textColorStyle = customTextColor ? { color: customTextColor } : {};
  const backgroundColorStyle = customBackgroundColor
    ? { backgroundColor: customBackgroundColor }
    : {};
  ...
  return (
    ...
    >
      ...
      >
        <header
          className={`w-fit border ${borderColorClass} ${
            customBorderRadius || "rounded-2xl"
          } p-2 px-4 md:px-8 flex items-center justify-center backdrop-blur-sm ${headerClassName}`}
          style={{ ...backgroundColorStyle, ...borderColorStyle }}
        >
          {showHomeButton && (
            <button
              onClick={scrollToTop}
              className={`group relative inline-flex items-center justify-center p-1 mr-0 md:mr-6 transition-colors ${homeHoverBgClass} rounded-[2px] overflow-hidden`}
              aria-label="Home"
            >
              <span
                aria-hidden
                className={`absolute inset-0 z-0 ${highlightBgClass} origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-700 ease-out rounded-[2px]`}
                style={highlightBgStyle}
              />
              <HomeIcon
                className={`relative z-10 h-4 w-4 ${homeIconColor} ${homeIconHoverColor} transition-colors`}
          ...
          >
            {box.ready && (
              ...
              >
                <span
                  className={`tabular-nums select-none ${textColorClass}`}
                  style={
                    customBracketColor
                      ? { color: customBracketColor }
                      : textColorStyle
                  }
                >
                  [
